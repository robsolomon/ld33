...
namespace CSharpSynth.Synthesis
{
    public class StreamSynthesizer
    {
        ...
        public bool LoadBank(string filename)
        {
			...
			if (CSharpSynth.Unity.EditorConfiguration.DEBUG_BANK_LOAD_SUMMARY)
				Debug.Log ("USynth MIDI Bank Load: " + filename);
            BankManager.addBank(new InstrumentBank(sampleRate, filename));
            SwitchBank(BankManager.Count - 1);
			if (CSharpSynth.Unity.EditorConfiguration.DEBUG_BANK_LOAD_SUMMARY)
				Debug.Log ("USynth MIDI Bank Load: Finished loading in " + (Time.realtimeSinceStartup - timestamp) + " second(s).");
            return true;
        }
        ...
        private void setupSynth()
        {
            ...
            if (sampleRate < 8000 || sampleRate > 48000)
            {
                ...
                //UnitySynth
                Debug.LogWarning("-----> Invalid Sample Rate! Changed to---->" + sampleRate);
				Debug.LogWarning("-----> Invalid Buffer Size! Changed to---->" + 50 + "ms");
            }
            if (polyphony < 1 || polyphony > 500)
            {
                polyphony = 40;
				Debug.LogWarning("-----> Invalid Max Poly! Changed to---->" + polyphony);
            }
            if (maxnotepoly < 1 || maxnotepoly > polyphony)
            {
                maxnotepoly = 2;
				Debug.LogWarning("-----> Invalid Max Note Poly! Changed to---->" + maxnotepoly);
            }
            ...
            {
                audioChannels = 1;
				Debug.LogWarning("-----> Invalid Audio Channels! Changed to---->" + audioChannels);
            }
